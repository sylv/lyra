use crate::{
    AppState, RequestAuth, config::get_config, entities::assets as assets_entity, error::AppError,
};
use axum::{
    Router,
    body::Body,
...
async fn get_asset(
    ...
    let asset = assets_entity::Entity::find_by_id(asset_id)
        .one(&state.pool)
        .await?
    ...
    let Some(source_url) = asset.source_url else {
        return Err(anyhow::anyhow!("asset is not remotely backed yet").into());
    };

    proxy_image_url(&source_url, &params).await
}
...
async fn proxy_image(
    ...
    proxy_image_url(&url, &params).await
}

pub async fn proxy_image_url(url: &str, params: &TranscodeParams) -> Result<Response, AppError> {
...
fn is_url_allowed(url: &str) -> bool {
    ALLOWED_URLS.iter().any(|pattern| url.starts_with(pattern))
}
