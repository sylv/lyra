...
fn calc_fingerprint(
    ...
    let mut ffmpeg = StdCommand::new(ffmpeg_path)
        .arg("-hide_banner")
        .arg("-loglevel")
        .arg("error")
        .arg("-nostdin")
...
async fn detect_intros_with_tools_async(
    ffmpeg_path: &str,
    ffprobe_path: &str,
    input_files: &[IntroDetectionInputFile],
    cancellation_token: Option<&CancellationToken>,
    ...
    for (index, input_file) in input_files.iter().enumerate() {
            ...
            decode_fingerprint_cache(input_file.fingerprint_cache.as_deref())
        {
            ...
            let Some(fingerprint) = calc_fingerprint_async(
                ffmpeg_path,
                ffprobe_path,
                path,
                &config,
...
async fn calc_fingerprint_async(
    ...
    let fingerprint_worker = std::thread::spawn(move || -> anyhow::Result<Vec<u32>> {
        let config = chromaprint_config();
        let mut printer = Fingerprinter::new(&config);
        printer
            .start(FINGERPRINT_SAMPLE_RATE, FINGERPRINT_CHANNELS)
    ...
    let fingerprint = fingerprint_worker
        .join()
        .map_err(|_| anyhow::anyhow!("fingerprint worker panicked"))??;
    Ok(Some(fingerprint))
}
