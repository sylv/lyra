...
use crate::{
    ...
    state::{
        AppState, KeyframePolicy, StreamProfileKey, build_master_playlist, build_stream_profiles,
        create_process_segment_dir, load_keyframes_if_needed_with_policy, prepare_segments_root,
        prepare_segments_root_at, probe_streams,
    },
};
...
pub fn default_profiles() -> Vec<Arc<dyn Profile>> {
    vec![
...
pub fn build_packager_state(
    input: &FsPath,
    profiles: &[Arc<dyn Profile>],
    segments_root: Option<&FsPath>,
) -> Result<Arc<AppState>> {
    build_packager_state_with_keyframe_policy(
        input,
        profiles,
        segments_root,
...
pub fn build_packager_state_with_keyframe_policy(
    input: &FsPath,
    profiles: &[Arc<dyn Profile>],
    segments_root: Option<&FsPath>,
...
pub fn build_packager_state_with_defaults(input: &FsPath) -> Result<Arc<AppState>> {
    let profiles = default_profiles();
    build_packager_state(input, &profiles, None)
}

pub fn build_packager_router(state: Arc<AppState>) -> Router {
...
pub async fn serve_packager(state: Arc<AppState>, bind_addr: &str) -> Result<()> {
    let app = build_packager_router(state);
